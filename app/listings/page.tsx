...
function ListingsPageContent() {
  ...
  const initialMaxPrice = searchParams.get("maxPrice") ? parseInt(searchParams.get("maxPrice")!, 10) : 50000;
  ...
  }, [searchQuery, selectedCategory, selectedLocation, selectedMake, selectedModel, priceRange, conditions, sortBy, router]);


  // Apply filters whenever listings or filter criteria change
  useEffect(() => {
    applyFilters();
  }, [listings, searchQuery, selectedCategory, selectedLocation, selectedMake, selectedModel, priceRange, conditions, sortBy]);
  ...
  const availableModels = selectedMake 
    ? Array.from(new Set(listings.filter(l => l.make === selectedMake).map(l => l.model).filter(Boolean))).sort()
    : [];
        ...
        <aside className={`md:col-span-1 ${showFilters ? "block" : "hidden"} md:block`}>
          <div className="sticky top-20 space-y-6 rounded-lg border bg-card p-6 shadow">
            ...
            <div>
              ...
              <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                ...
                <SelectContent>
                  ...
                  {availableCategories.map(category => <SelectItem key={category} value={category}>{category.charAt(0).toUpperCase() + category.slice(1)}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div>
              ...
              <Select value={selectedModel} onValueChange={setSelectedModel} disabled={!selectedMake || availableModels.length === 0}>
            ...
            <div>
              ...
              <div className="flex justify-between text-sm text-muted-foreground">
                ...
                <span>${priceRange[1]}{priceRange[1] === 50000 ? "+" : ""}</span>
              </div>
            </div>
